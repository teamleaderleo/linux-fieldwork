...
# Probe-only cache pressure knob. It never enters the product candidate diff.
...
# Probe-only controls: make one cached refblock clean without flushing its
# dirty sibling, then reduce cache capacity so inserting the next region must
# evict one of the two transaction-touched entries.
...
rc_new = '''        self.ref_table.set_dirty(ref_table_dirty);
    ...
    pub(super) fn flush_cached_refblock_for_test(
        ...
        let addr = *self.ref_table.get(table_index).ok_or_else(|| io::Error::from_raw_os_error(EINVAL))?;
        let block = self
            .refblock_cache
            .get_mut(table_index)
...
probe = r'''
    ...
    fn refblock_journal_rolls_back_dirty_sibling_after_eviction() {
        ...
        // The sparse extent is test control only; every metadata transition below
        // uses the production allocator/refcount/cache paths.
        ...
        // First, build a coherent successful cross-region state:
        // table0 -> Y in region1; table1 -> Z in region1; X/Y/Z all rc1.
        let target1 = 0x40000;
        let y = region_span;
        ...
        assert_eq!(table_after_first[0], y);
        assert_eq!(table_after_first[1], z);

        // Reload target1 so table0/table1 are cached again, then dirty only
        // table1 by changing a still-free low slot and restoring it before the
        // second transaction. The logical value returns to zero but table1's
        // cached state remains dirty.
        {
            let super::QcowState {
                refcounts,
                raw_file,
            ...
            assert_eq!(refcounts.get_cluster_refcount(raw_file, target1).unwrap(), 1);
            assert_eq!(refcounts.get_cluster_refcount(raw_file, y).unwrap(), 1);
        }

        // Force table0 clean while keeping table1 dirty. First mutate and
        // restore one region1 entry so its cache is dirty with unchanged
        // logical contents.
        let region1_scratch = y + 2 * cluster_size;
        inner
            .set_cluster_refcount_track_freed(region1_scratch, 1)
            .unwrap();
        inner
            .set_cluster_refcount_track_freed(region1_scratch, 0)
            .unwrap();
        inner.unref_clusters.retain(|addr| *addr != region1_scratch);
        inner
            .refcounts
        ...
        // The second transaction relocates clean table0 to A. Releasing Y
        // mutates the pre-existing dirty table1. Owning A inserts table2 and,
        // at capacity 2, must evict one touched cached block. Owning B then
        // enters empty region3 and ENOSPCs because no allocation remains.
