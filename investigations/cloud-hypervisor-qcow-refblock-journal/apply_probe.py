...
text = path.read_text()
marker = "refblock_recursive_enospc_journal_rolls_back"
if marker in text:
...
probe = r'''
    ...
    #[test]
    fn refblock_recursive_enospc_journal_rolls_back() {
        ...
        eprintln!(
            "REFBLOCK_JOURNAL_ROLLBACK post_error target={allocated_target:#x} target_refcount={target_refcount} replacement={replacement_refblock:#x} replacement_refcount={replacement_refcount} replacement_free={} unref_count={}",
            inner.avail_clusters.contains(&replacement_refblock),
            inner.unref_clusters.len()
        );
        assert_eq!(target_refcount, 0);
        ...
        let table_after_rollback = inner
            ...
            .unwrap();
        eprintln!(
            "REFBLOCK_JOURNAL_ROLLBACK post_sync table0={:#x} table1={:#x} old={old_refblock:#x}",
        ...
        eprintln!(
            "REFBLOCK_JOURNAL_ROLLBACK reopened table0={:#x} replacement_refcount={replacement_refcount_after} replacement_free={} free_tail={:#x?}",
        ...
        let table_after_reuse = reopened
            ...
            .unwrap();
        eprintln!(
            "REFBLOCK_JOURNAL_ROLLBACK allocator_reuse reused={reused:#x} table0={:#x}",
            table_after_reuse[0]
        );
    ...
    #[test]
    fn refblock_recursive_journal_success_owns_chain_and_tracks_old_block() {
        let cluster_size: u64 = 1 << 16;
        let refcount_bits: u64 = 16;
        let refcount_block_entries = cluster_size * 8 / refcount_bits;
        ...
        eprintln!(
            "REFBLOCK_JOURNAL_SUCCESS target={allocated_target:#x}:{target_refcount} y={replacement_region0:#x}:{y_refcount} z={replacement_region1:#x}:{z_refcount} old={old_refblock:#x}:{old_refcount} old_tracked={}",
        ...
        assert!(inner.unref_clusters.contains(&old_refblock));
        ...
        eprintln!(
            "REFBLOCK_JOURNAL_SUCCESS reopened table0={:#x} table1={:#x} target={target_after} y={y_after} z={z_after} old={old_after} old_free={}",
            table_after_reopen[0],
