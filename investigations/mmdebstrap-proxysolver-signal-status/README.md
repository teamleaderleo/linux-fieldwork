...
This follow-up restores the signal's default disposition and signals the wrapper itself after stdout/dump files have closed.
...
Catchable signals have their default disposition restored because Python ignores or handles some signals differently from a freshly executed child. SIGKILL and SIGSTOP cannot have a handler installed but can still be sent to the wrapper.

## Regression

A disposable fake solver writes one complete line, records its PID, flushes through the shell, and terminates itself with SIGTERM.
...
- stdout and dump bytes to retain the flushed line in both cases;
