...
The fixture contains one large-ID member that requires PAX numeric fields and one ordinary header-sized control. The unmodified source must shift only the ordinary member while the stale PAX values keep the large member unchanged. The candidate removes `uid` and `gid` from `member.pax_headers` after shifting so Python regenerates consistent values. Payload equality and a `-7` round trip are required.
