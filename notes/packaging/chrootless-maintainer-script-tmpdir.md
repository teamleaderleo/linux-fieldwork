...
The retained candidate regression applies a one-line allowlist patch to a temporary source copy, requires the unmodified negative control to use host `/tmp`, and requires the candidate to use `<target>/tmp`, mode `1777`, with cleanup, fresh rerun, and fakeroot coverage.

Exact candidate workflow results are pending.
