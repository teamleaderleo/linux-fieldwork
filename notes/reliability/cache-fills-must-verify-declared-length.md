...
`caching_proxy.py` reads upstream responses in 64 KiB chunks. Issue #101 records that fixed-size `HTTPResponse.read()` can return EOF without raising on a short declared response, and the candidate validates byte counts before the atomic writer publishes.
