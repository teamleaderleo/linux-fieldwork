...
**State:** defect and current source candidate are well evidenced; pre-publish review found a memory-scaling regression; streaming repair staged for review.

- Fieldwork investigation: `investigations/cloud-hypervisor-kvm-dirty-log-granule/`
- Controlling issue: `teamleaderleo/linux-fieldwork#617`
- Exact Cloud Hypervisor base: `1af93ac7035cda77cd87b0c18b1134ebb0928052`
- Signed source candidate: `2038c3cb262ac27604f647729557893a61510f99`
- Source tree: `6efdf4682cc5035c2ce76bda9aa54688ffe34934`
- Real 4K KVM migration run/job: `31567234922` / `94021476274` — success

The repair carries dirty-bitmap data together with its byte granularity so KVM can use the host base-page unit while MSHV keeps its fixed 4K contract. Synthetic 4K/16K/64K conversion, mismatch, bounds, and tail-bit controls are green, as is real 4K KVM migration. Real non-4K KVM migration remains the evidence gap.

Do not package the signed candidate as-is. Pre-publish review found that it materializes an extra full `Vec<u64>` even though Cloud Hypervisor previously removed that pattern because dirty bitmaps can be hundreds of MiB. `STREAMING_REPAIR.patch` preserves tail validation while restoring iterator-based merge/conversion. Review and execute that streaming delta before any maintainer-facing packet.
