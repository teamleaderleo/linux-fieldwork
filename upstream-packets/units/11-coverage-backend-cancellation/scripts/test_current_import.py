...
GROUP_PATCH = (
    ROOT
    / "upstream-packets/units/11-coverage-backend-cancellation/patches/"
    / "0001-coverage-own-selected-backend-group.patch"
)
EXPECTED_SOURCE_BLOB = "9a522484aef05deae514a98e4b6adf5feb6c886d"
...
class CurrentImportCoverageProcessGroupTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        actual_blob = git_blob_sha(SOURCE)
        if actual_blob != EXPECTED_SOURCE_BLOB:
            raise AssertionError(
                f"coverage.py blob changed: expected {EXPECTED_SOURCE_BLOB}, "
                f"found {actual_blob}"
            )

        cls.work = tempfile.TemporaryDirectory(prefix="unit-11-current-import-")
        root = pathlib.Path(cls.work.name)
        cls.baseline = root / "baseline-coverage.py"
        cls.status_candidate = root / "status-only-coverage.py"
        cls.group_root = root / "group-owned"
        cls.group_candidate = cls.group_root / "coverage.py"
        ...
        print(f"source_blob={actual_blob}")
        print(f"patch_blob={git_blob_sha(GROUP_PATCH)}")
    ...
    def apply_patch(cls, tree: pathlib.Path, patch: pathlib.Path) -> None:
        ...
        print("patch_application=success fuzz=0")
