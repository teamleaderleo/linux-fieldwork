...
- Exact predecessor before this final handoff refresh: `1559e6e85c74153d00d96e867acda099cbdae3f5`
...
- Historical Linux Fieldwork candidate: PR #77 head `e65989feaac9a9cb89c49fe536c26fe9e9ee8cb7`, merge `4b9e24b0b20c1398dcae825310c6b7d0d5c273d0`
...
4. Identified canonical upstream and exact current base/source identity.
5. Confirmed the defect remains current.
6. Corrected the unit from `HOLD` to `ACTIVE` and corrected the destination from packaging Salsa to canonical Forgejo implementation upstream.
7. Re-read units 01, 15, and 16; none owns `TypeFilterAction` regular-class membership or blocks this unit's order.
8. Read upstream `tests/tarfilter-idshift`, `coverage.txt`, `coverage.py`, and `run_null.sh` and mapped the native test contract.
9. Added the native shell test, registry fragment, and exact-source Linux Fieldwork gate.
10. Opened internal draft PR #410 for hosted CI; no upstream contact occurred.
11. Probed Python 3.13.5: `REGTYPE == b"0"`, `AREGTYPE == b"\0"`, and both satisfy `TarInfo.isfile()`.
12. Verified Python USTAR round-trip preserves both bytes and payloads.
13. Verified GNU tar 1.35 lists and extracts both as ordinary regular files with exact payloads.
14. Ran the native shell test against a faithful minimal filter model: baseline failed with leaked `nul-regular`; candidate passed twice.
15. Verified GNU patch 2.8 is available locally; shellcheck/shfmt are not, so no local formatting claim is made.
16. Performed a bounded current Forgejo issue/pull-request overlap search; no visible equivalent work was found.
17. Recorded direct Git DNS failures as environment transport limitations, not product results.
18. Updated README, source map, deep dive, tests, decisions, drafts, and this handoff with exact findings and donuts.
...
- Draft PR: #410
- Latest exact predecessor head: `1559e6e85c74153d00d96e867acda099cbdae3f5`
- Latest observed run for that head: `30694037117`
...
1. Install `native/tests/tarfilter-regular-type-class` as executable `tests/tarfilter-regular-type-class`.
2. Add `Test: tarfilter-regular-type-class` to `coverage.txt`.
3. Apply the source patch with zero fuzz and zero offsets.
4. Run the real `coverage.py` shellcheck/shfmt path.
5. Run:
...
7. Clean and rerun the focused test immediately.
8. Compose with adjacent tarfilter candidates for one compatibility gate.
9. Review the complete final diff, executable mode, and refreshed overlap.
10. Move directly to `READY FOR AUTHORIZATION` when these technical gates pass.
