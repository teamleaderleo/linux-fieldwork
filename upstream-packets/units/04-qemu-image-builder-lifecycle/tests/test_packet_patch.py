...
DEFAULT_REPO = pathlib.Path(__file__).resolve().parents[4]
SOURCE = pathlib.Path(
    os.environ.get(
        "MMDEBSTRAP_QEMU_SOURCE",
        DEFAULT_REPO / "upstream/mmdebstrap/mmdebstrap-autopkgtest-build-qemu",
    )
)
...
class PacketPatchTest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cleanup_end = cls.added.index(
            '  return "$cleanup_status"\n', cleanup_start
        ) + len('  return "$cleanup_status"\n')
        cleanup = "cleanup() {\n" + cls.added[cleanup_start:cleanup_end] + "}\n"
        cls.functions = "\n".join(
            [
                extract_function(cls.added, "prepare_image"),
                extract_function(cls.added, "publish_image"),
                cleanup,
                extract_function(cls.added, "exit_cleanup"),
                extract_function(cls.added, "signal_exit"),
            ]
        )
        traps = [
            line
            for line in cls.added.splitlines()
            if line.startswith("trap exit_cleanup")
            or line.startswith("trap 'signal_exit")
        ]
        cls.traps = "\n".join(traps) + "\n"
    ...
    def test_upstream_paths_coordinates_and_single_publication(self) -> None:
        self.assertIn(
        ...
        self.assertEqual(
            self.patch.count('mv --no-target-directory -- "$IMAGE_TMP" "$IMAGE"'),
            1,
        )
        self.assertIn(
            'set -- "$@" -E "$EXTOPTS" "$IMAGE_TMP" "$SIZE"', self.patch
        )
        self.assertIn('/sbin/sfdisk "$IMAGE_TMP" <<EOF', self.patch)
        self.assertIn('dd if="$WORKDIR/fat" of="$IMAGE_TMP"', self.patch)

    def test_patch_applies_without_offset_or_fuzz_to_exact_imported_source(
        self,
    ) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="unit04-apply-") as td:
            ...
            checked = subprocess.run(
                ["sh", "-n", str(candidate)], capture_output=True, text=True
            )
            self.assertEqual(checked.returncode, 0, checked.stdout + checked.stderr)

    def test_failure_preserves_existing_and_absent(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = pathlib.Path(td)
            script = self.write_harness(root)
            image = root / "existing.img"
            image.write_bytes(b"sentinel")
            image.chmod(0o600)
            result = self.run_harness(script, root / "run1", "failure", image)
            ...
            result = self.run_harness(script, root / "run2", "failure", absent)
            self.assertEqual(result.returncode, 42)
            self.assertFalse(absent.exists())
            self.assertEqual(self.private_dirs(absent), [])

    def test_hup_int_term_and_rerun(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = pathlib.Path(td)
            script = self.write_harness(root)
            for sig, code in (
                ...
                image = root / f"{sig.name}.img"
                image.write_bytes(b"sentinel")
                runtime = root / f"run-{sig.name}"
                result = self.signal_run(script, runtime, "wait-before", image, sig)
                self.assertEqual(result.returncode, code)
                self.assertEqual(image.read_bytes(), b"sentinel")
                self.assertFalse((runtime / "after").exists())
                self.assertEqual(self.private_dirs(image), [])
                rerun = root / f"rerun-{sig.name}"
                result = self.run_harness(script, rerun, "success", image)
                self.assertEqual(result.returncode, 0)
                self.assertEqual(image.read_bytes(), b"complete")

    def test_success_mode_and_post_publication_term(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = pathlib.Path(td)
            script = self.write_harness(root)
            image = root / "success.img"
            image.write_bytes(b"sentinel")
            result = self.run_harness(script, root / "success", "success", image)
            self.assertEqual(result.returncode, 0)
            self.assertEqual(image.read_bytes(), b"complete")
            self.assertEqual(stat.S_IMODE(image.stat().st_mode), 0o644)
            image2 = root / "after.img"
            image2.write_bytes(b"sentinel")
            result = self.signal_run(
                script, root / "after", "wait-after", image2, signal.SIGTERM
            )
            self.assertEqual(result.returncode, 143)
            self.assertEqual(image2.read_bytes(), b"complete")

    def test_cleanup_precedence(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = pathlib.Path(td)
            script = self.write_harness(root)
            result = self.run_harness(
                script, root / "success", "cleanup-only", root / "a.img", True
            )
            self.assertEqual(result.returncode, 74)
            result = self.run_harness(
                script, root / "failure", "failure", root / "b.img", True
            )
            self.assertEqual(result.returncode, 42)

    def test_trailing_slash_rejected_before_mktemp(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            result = subprocess.run(
                ["sh", str(script), str(log), f"{dest}/"],
                capture_output=True,
                text=True,
            )
            self.assertEqual(result.returncode, 1)
            self.assertIn("invalid image path", result.stderr)
