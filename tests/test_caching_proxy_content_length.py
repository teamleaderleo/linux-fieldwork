...
NEGATIVE_LENGTH_BODY = b"NEGATIVE-CONTENT-LENGTH"
...
class RecoveringUpstream(http.server.BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        ...
        if self.path.endswith("/negative"):
            ...
            self.end_headers()
            self.wfile.write(NEGATIVE_LENGTH_BODY)
            self.wfile.flush()
            self.close_connection = True
            return
...
class CachingProxyContentLengthTest(unittest.TestCase):
    ...
    def test_candidate_rejects_negative_content_length_before_publication(self) -> None:
        module = self.load_module(self.candidate_source)
        oldcache = self.work / "negative-old"
        newcache = self.work / "negative-new"
        oldcache.mkdir()
        newcache.mkdir()
        final_path = newcache / "pool/negative"

        with self.upstream_server() as upstream, self.proxy_server(
            module, oldcache, newcache
        ) as proxy_port:
            response = self.proxy_get(proxy_port, upstream.port, "pool/negative")

        self.assertEqual(response.status, 502)
        self.assertEqual(upstream.request_count, 1)
        self.assertFalse(final_path.exists())
        pool = newcache / "pool"
        self.assertEqual(list(pool.glob(".negative.*")) if pool.exists() else [], [])
    ...
    def test_candidate_source_contract_and_compilation(self) -> None:
        ...
        self.assertIn("expected_length < 0", candidate)
