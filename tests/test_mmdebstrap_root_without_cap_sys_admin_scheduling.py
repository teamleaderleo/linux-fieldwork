...
def should_skip_for_host_apt(test_paragraph: str, use_host_apt_config: bool) -> bool:
    return (
        "Needs-APT-Config: true" in test_paragraph and use_host_apt_config
    )
...
class RootWithoutCapSysAdminSchedulingTest(unittest.TestCase):
    ...
    def test_candidate_moves_case_out_of_host_apt_hook_phase(self) -> None:
        ...
        self.assertFalse(should_skip_for_host_apt(baseline, True))
        self.assertTrue(should_skip_for_host_apt(candidate, True))
        self.assertFalse(should_skip_for_host_apt(candidate, False))
