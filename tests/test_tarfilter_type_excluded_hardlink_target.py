...
class TarfilterTypeExcludedHardlinkTargetTest(unittest.TestCase):
    @classmethod
    ...
    def run_filter(
        self, archive: bytes, excluded_type: str
    ) -> subprocess.CompletedProcess[bytes]:
        return subprocess.run(
            [
                sys.executable,
                str(self.source),
                f"--type-exclude={excluded_type}",
            ],
            input=archive,
    ...
    def test_excluding_regular_target_retains_dangling_hardlink(self) -> None:
        archive = self.archive_bytes()
        with tempfile.TemporaryDirectory(prefix="tarfilter-type-hardlink-") as td:
            ...
            filtered = self.run_filter(archive, "REGTYPE")
            self.assertEqual(
                filtered.returncode,
                0,
    ...
    def test_excluding_hardlink_leaves_extractable_regular_target(self) -> None:
        archive = self.archive_bytes()
        with tempfile.TemporaryDirectory(prefix="tarfilter-type-control-") as td:
            root = pathlib.Path(td)
            filtered = self.run_filter(archive, "LNKTYPE")
            self.assertEqual(
                filtered.returncode,
                0,
                filtered.stderr.decode("utf-8", "replace"),
            ...
            self.assertEqual(
                (filtered_root / "root/base").read_bytes(),
                b"hard-link-payload\n",
            )
            self.assertFalse((filtered_root / "root/peer").exists())
