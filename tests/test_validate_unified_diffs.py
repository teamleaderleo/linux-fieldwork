...
"""
    ...
    def test_deleted_line_equal_to_mail_signature_separator_is_valid(self) -> None:
        patch = (
            "--- a/file\n"
            "+++ b/file\n"
            "@@ -1,2 +1 @@\n"
...
"""
    ...
    def test_empty_git_file_section_is_rejected(self) -> None:
        result = validate_text("diff --git a/file b/file\n")
        self.assertEqual(len(result.findings), 1)
        self.assertIn("has no textual, binary, or metadata-change payload", result.findings[0].message)
    ...
    def test_symbolic_link_patch_path_is_rejected(self) -> None:
        ...
        self.assertEqual(len(result.findings), 1)
        self.assertIn("symbolic link", result.findings[0].message)
