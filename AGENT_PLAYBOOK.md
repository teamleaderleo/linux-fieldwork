# Detailed owned-fork agent policy

This is the task-selected detailed policy preserved from the former automatic `AGENTS.md`. The root router owns the universal internal-authority and external-interaction boundary. Load this file when a task needs the complete owned-fork execution or upstream-candidate procedure.
