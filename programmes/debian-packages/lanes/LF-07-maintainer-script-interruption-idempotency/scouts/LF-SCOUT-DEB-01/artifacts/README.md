...
The runner requires root, `dpkg`, `dpkg-deb`, BusyBox at `/usr/bin/busybox`, and ordinary GNU userland tools. It builds the package in a temporary work directory, creates four minimal chroot roots, performs one clean installation plus three interrupted installations, reruns each half-configured package with `dpkg --configure`, and rewrites `results/`. It asserts the expected return codes, package states, convergence outcomes, and registry-line counts; any changed outcome makes the runner exit nonzero.
...
- `run-probe.sh` — builds the package, creates disposable roots, injects `SIGTERM`, snapshots state, reruns configuration, compares outcomes, and enforces the expected matrix.
...
- `results/assertions.tsv` — every expected value, observed value, and pass/fail result.
...
- `results/probe-transcript.txt` — installation, interruption, half-configured-state, recovery, summary, and assertion transcript.
